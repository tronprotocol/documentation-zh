...
状态数据快照可以通过执行LiteFullNodeTool.jar进行获得（请参考：[如何使用LiteFullNode Tool](../../using_javatron/litefullnode.md)）。
...
1、增加了两个新的系统合约MarketSellAssetContract和MarketCancelOrderContract用于支持TRX、TRC-10资产在链上去中心化交易所进行交易。
