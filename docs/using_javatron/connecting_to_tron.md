...
种子节点作为引导节点，也属于被发现的节点。如果节点发现没有开启，当前节点不会主动连接这些种子节点。
...
### 网络ID
在被发现的节点中，只有和当前网络ID相同的节点才会成为可连接节点。比如，如果在种子节点列表中误配置了其他网络的节点，在节点发现过程中，由于对方节点返回的网络ID和当前节点配置的类型不一样，当前节点会将其判定为不可连接。
...
![image](https://raw.githubusercontent.com/tronprotocol/documentation-zh/master/images/network_topology2.png)
