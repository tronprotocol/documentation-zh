...
* [本地事件插件订阅](#local-event-plugin-subscription-recommended)
* [内置消息队列（ZeroMQ）订阅](#built-in-message-queue-subscription-zeromq)

## 本地事件插件订阅（推荐） { #local-event-plugin-subscription-recommended }
...
#### 配置事件订阅 { #configuring-event-subscription-rules }
...
### MongoDB 事件订阅插件部署与使用指南 { #use-mongodb }
...
#### 安装与配置 MongoDB { #deploying-the-mongodb }
...
**注意**：x86_64架构上请使用JDK 8（推荐最新的小版本），arm64架构上请使用JDK 17
...
mongo.username=${MONGO_USERNAME}  # 示例: tron
mongo.password=${MONGO_PASSWORD}  # 生产环境中务必使用强密码
...
#### 启动服务与验证数据 { #starting-and-verifying }
...
## 内置消息队列订阅（ZeroMQ） { #built-in-message-queue-subscription-zeromq }
