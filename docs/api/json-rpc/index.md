...
> **默认关闭**：`config.conf` 中 `jsonrpc {}` 块的所有开关默认注释掉，`Args` 中 `httpFullNodeEnable` / `httpSolidityEnable` 均为 `false`（见 `Args.java:206`）。需在配置中显式 `httpFullNodeEnable = true` / `httpSolidityEnable = true` 才会随节点启动。Solidity JSON-RPC 服务还要求当前进程为 FullNode（不是独立的 SolidityNode 进程，见 `JsonRpcServiceOnSolidity.java:20`）。

URL 路径恒为 `/jsonrpc`（见 `FullNodeJsonRpcHttpService.java:31`）。

## 协议约定

- **传输**：仅 `POST`，请求体为 [JSON-RPC 2.0](https://www.jsonrpc.org/specification) 格式：`{"jsonrpc":"2.0","method":"...","params":[...],"id":1}`。
- **HTTP 状态码**：恒为 200，业务错误通过响应体 `error` 字段表达（见 `JsonRpcServlet.java:48-58`）。
...
| `-32005` | `JsonRpcExceedLimitException` / `JsonRpcTooManyResultException` | 触发限额（block filter 数、`eth_getLogs` 结果或 block range） |
