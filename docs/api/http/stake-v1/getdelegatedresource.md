...
| 字段 | 方法 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| `fromAddress` | GET | string | 是 | 资源出借方地址 |
| `fromAddress` | POST | string | 否 | 资源出借方地址；省略时使用空 bytes 并返回空结果 |
| `toAddress` | GET | string | 是 | 资源接收方地址 |
| `toAddress` | POST | string | 否 | 资源接收方地址；省略时使用空 bytes 并返回空结果 |
