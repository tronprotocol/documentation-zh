...
| 字段 | 方法 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| `address` | GET / POST | string | 否 | 投票账户地址；省略或为空时返回 `reward: 0` |
| `visible` | GET / POST | bool | 否 | 无影响（servlet 通过 `41` 前缀自动识别地址格式，响应无 bytes 字段） |
| `int64_as_string` | GET | bool | 否 | 仅 GET；为 `true` 时将 `reward` 作为 JSON 字符串返回 |
...
| GET / POST | `address` 解析失败（非法 hex / base58） | `{"Error": "INVALID address, <details>"}` |
