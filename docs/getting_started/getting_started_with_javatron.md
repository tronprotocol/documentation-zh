...
`java-tron` 是用 Java 语言编写的波场（TRON）网络客户端。运行 `java-tron` 会将您的计算机转变为一个完整的波场节点，并接入一个由众多节点组成的去中心化网络。在这个网络中，信息由节点间直接共享，而非通过中心化服务器管理。当超级代表（Super Representative）的节点产出一个新区块后，会将其广播至网络中的其他节点。每个节点在收到新区块时会进行验证，验证通过后便将其存入本地数据库。

`java-tron` 通过同步区块来持续更新其“状态”——即波场网络上所有账户的实时余额。

波场网络上有两种主要类型的账户：
- **外部账户 (Externally Owned Accounts)**: 由用户通过公私钥对控制。公钥用于派生出独一无二的账户地址，而私钥则用于安全地签署和授权交易。您在本教程中创建的便是这种类型的账户。
- **合约账户 (Contract Accounts)**: 当收到交易时，这类账户会执行其中包含的智能合约代码。

要开始与波场网络交互，您首先需要创建一个外部账户。
