...
TRC-10 token 转账的消耗逻辑最为特殊，它引入了“token 发行方”作为潜在的费用承担者：

1. 消耗发行方带宽 (优先)：系统首先会尝试消耗由 token 发行方为该 token 预付的带宽。这需要同时满足三个条件（只有当这三项检查全部通过时，才会扣除发行方的带宽。否则，进入下一步，由转账发起者自己支付）：
    - token 发行方剩余额度充足
        - 查询方式：[/wallet/getassetissuebyaccount](../api/http/asset/getassetissuebyaccount.md) 
        - 计算公式： `public_free_asset_net_limit - public_free_asset_net_usage`
        - 含义：token 发行方能为该 token 转账支付的剩余额度
    - 转账发起者的个人额度充足
        - 查询方式：[/wallet/getaccountnet](../api/http/account/getaccountnet.md) 
        - 计算公式： `assetNetLimit['assetID'] - assetNetUsed['assetID']`
        - 含义：该 token 持有者还能使用的由发行方提供的免费带宽额度
    - token 发行方通过质押获取的带宽充足
        - 查询方式：[/wallet/getaccountnet](../api/http/account/getaccountnet.md) 
...
### 如何填写 `fee_limit` (用户必读) { #set-fee-limit }
...
- 假设合约 `C` 上一次成功执行时，消耗了 18000 Energy。通过调用 [estimateenergy](../api/http/smart-contract/estimateenergy.md) 接口进行预估，假设本次执行消耗的 Energy 上限约为 20,000 Energy；
...
### TRON 网络能量消耗机制 { #energy-mechanism }
...
- 当 `energy_factor` 大于 `0` 时，表示该合约已经是热门合约，调用该合约时将消耗额外的能量。合约的能量消耗放大系数可以通过 [getcontractinfo](../api/http/smart-contract/getcontractinfo.md) 接口查询。
...
撤销操作并非随机，而是按比例公平地从您投票给的每一个超级代表/超级代表合伙人身上撤回选票。
...
* **被代理方该资源的总质押数额**：指被代理方持有的某项资源（能量或带宽）的质押总量（Stake 1.0 和 Stake 2.0 自身质押以及他人代理总和）。可通过 [wallet/getaccount](../api/http/account/getaccount.md) 接口查询。
* **被代理方的未恢复资源量**：指被代理方账户中当前已消耗且正在处于恢复周期内的资源数量。可通过 [wallet/getaccount](../api/http/account/getaccount.md) 或 [wallet/getaccountresource](../api/http/account/getaccountresource.md) 接口查询。
