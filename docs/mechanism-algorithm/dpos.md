...
1. 初始准备: 当区块链系统启动时，会发行一定数量的 **token**（例如 TRON 的 TRX）。
2. 成为候选人: 节点可以消耗一部分 token 来申请成为区块生产者候选人。
3. 社区投票: 网络中任何持有 token 的节点都可以为这些候选人投票。
...
|TRX| 由 TRON 区块链系统发行并在系统中流通的权益 token，代号为 **TRX**。|
...
|出块顺序（记账顺序）| 27 个区块生产者按照得票数量从高到低进行排序，以此确定其生产区块的顺序。当两个 SR 的得票数相同时，使用其地址作为决胜依据，以产生确定的顺序。|
...
|出块轮（Epoch）|  TRON 设定每 **6 小时**为一个出块轮，称为一个 **Epoch**。在相邻两个出块轮的交界处，系统会执行一次**维护期**，以确定下一个出块轮的出块顺序。|
|维护期|在每个出块轮的交界处，系统会触发维护：统计本出块轮内累计的选票、重新选出 27 个 SR，并更新下一个出块轮的出块顺序。维护期间，产块会**跳过 2 个槽位**（**6 秒钟**），因此在该区间内不会产生新区块。由于 24 小时有 4 个出块轮，因此每天自然有 4 个维护期。|
...
此处对未确认状态区块的“认可”方式是：其他超级代表在该区块之后生产后续区块。例如，如图 d 所示，当超级代表 C 生产了第 103 块后，超级代表 E 在第 103 块的基础上生产了第 104' 块，超级代表 G、A、B 分别生产了第 105'、106'、107' 块，这些实质上都是第 103 块的后续区块，因此也是对 C 生产的第 103 块的认可。由此可知，当高度为 121' 的区块被生产出来时，第 103 块就成为了固化块，因为它此时已经有了 18 个后续区块。需要强调的是：生产这 18 个区块的超级代表必须**互不相同**，并且与生产第 103 个区块的超级代表也**不同**，这样就保证这个区块获得了 19 个不同的 SR 的确认，19 个 SR 包括区块的生产者。
...
下方所列的奖励数额为通过链上提案调整后的**当前主网值**，并非协议硬编码，未来仍可通过提案再次调整（详见[基于提案的参数调整](#proposal-based-parameter-adjustment)）。
...
<a id="proposal-based-parameter-adjustment"></a>

## 基于提案的参数调整

DPoS 的一个重要特性是，任何系统参数的调整都可以通过**链上提案**发起。SR 通过对提案的投票来决定提案是否生效。这种机制的好处在于，在链上调整参数时，**无需进行硬分叉升级**。

目前 TRON 的网络参数及其值，以及过往的提案记录，请参考 [TRONSCAN 委员会](https://tronscan.org/#/sr/committee) 页面。
